...
gff_source = '7_Expression/New_filtering/GFF'
wgs_source = '7_Expression/New_filtering/WGS'
fasta_source = '7_Expression/New_filtering/FASTA'
...
def main():
    ...
    for root, dirs, filenames in os.walk(fasta_source):
        for f in filenames:
            ...
            new_source = "7_Expression/New_filtering/Filtered_FASTA/"
...
def get_utr_bed(name, source, file):
    ...
    #Output bed
    bed_path = os.path.abspath("../2_Project_FSM/7_Expression/New_filtering/BED/" + name + ".bed")
    ...
    for i in list[1:]:
        if 'gene' in i and '\tCDS\t' in i:
            ...
            if '\tgene\t' in split[1]:
                ...
                #Next, get Gene ID
                name_split = i.split('\tCDS\t')[-1]
                next = name_split.split('protein_id=')
                protein_id = next[1].split('\n')[0]
                ...
                if gene_id in filtered_ids:
                    line = "\t".join([chromosome, start, stop, protein_id + "; " + 'unknown_exon', '.', strand, "\n"])
                    total += line
    ...
    for root, dirs, filenames in os.walk(source):
        for f in filenames:
            if name in f:
                ...
                genome_fasta = os.path.abspath("../2_Project_FSM/7_Expression/New_filtering/WGS/" + f)
